<?php get_header( );?>
<body <?php body_class();?>>
  
<!-- home section -->
<div class="home vh-80 d-flex align-items-center" id="home">
  <div class="container">
    <div class="row">
      <div class="col-lg-12 mx-auto text-center py-5">
        <ul class="slider">
          <li><img src="<?php echo esc_url( get_theme_file_uri('img/IMG_1177.jpg'));?> " alt="銀山"></li>
          <li><img src="<?php echo esc_url( get_theme_file_uri('img/IMG_1648.jpg'));?> " alt="袋田"></li>
          <li><img src="<?php echo esc_url( get_theme_file_uri('img/IMG_1676.jpg'));?> " alt="八ヶ岳"></li>
          <li><img src="<?php echo esc_url( get_theme_file_uri('img/IMG_1750.jpg'));?> " alt="富士山"></li>
          <li><img src="<?php echo esc_url(get_theme_file_uri('img/IMG_1990.jpg'));?> " alt="忠霊塔"></li>
          <li><img src="<?php echo esc_url( get_theme_file_uri('img/IMG_2113.jpg'));?> " alt="精進湖"></li>
          <li><img src="<?php echo esc_url( get_theme_file_uri('img/IMG_2597.jpg'));?> " alt="山中湖"></li>
          <li><img src="<?php echo esc_url( get_theme_file_uri('img/IMG_3046.jpg'));?> " alt="館山"></li>
        </ul>
        <h1>MASIDE</h1>
        <p>
          Web <span class="design">Design!!</span> 
        </p>
      </div>
    </div>
  </div>
</div>
<!--About section  -->
<div class="about text-center" id="about">
  <div class="container px-4 px-lg-5">
    <div class="row gx-4 gx-lg-5 justify-content-center">
      <div class="col-lg-8">
        <h2 class="text-white mb-4">Created with Bootstrap and converted to WordPress</h2>
        <p class="text-white-50">
          本サイトはBootstrapを使用して作成し、Word Press化しています。
        </p>
      </div>
      <img class="img-fluid about_img" src="<?php echo esc_url(get_theme_file_uri('/img/3040945_m.jpg'));?>" alt="パソコン">
    </div>
  </div>
</div>


<!-- Profile section -->
<div class=" text-center bg-light" id="profile">
  <div class="container px-4 px-lg-5">
    <!-- profile -->
    <div class="row gx-0 mb-4 mb-lg-5 align-text-center ">
      <div class="col-xl-8 col-lg-7">
        <img class="img-fluid mb-3 mb-lg-0 profile_img" src="<?php echo esc_url(get_theme_file_uri('img_3142.jpeg'));?>" alt="Example Image">
      </div>
      <div class="col-xl-4 col-lg-5 aline-self-center">
        <div class="featured-text text-center">
          <a href="<?php echo esc_url('http://portfolio.local/sakura/');?>"><h4>Profile</h4></a>
          <p class="text-black-50" >
            山梨県在住。現在会社員を務める傍ら、Webサイト制作におけるコーディング、レスポンシブ対応を行なっています。<br>
            会社員の経験を活かし、丁寧な仕事を心がけております。お気軽にお問合せください。
          </p>
        </div>
      </div>
    </div>
    
    <!-- work one -->
    <div class="row gx-0 mb-5 mb-lg-0 justify-content-center" id="works">
      <div class="col-lg-6">
        <a href="http://portfolio.local/works/" target="_blank">
          <img class="img-fluid work_one_img w-100 " src="<?php echo esc_url(get_theme_file_uri('img/SocialTech.png'));?> " alt="works image">
        </a>
      </div>
      <div class="col-lg-6">
        <div class="bg-black text-center h-100 work">
          <div class="d-flex h-100">
            <div class="project-text w-100 my-auto text-center text-lg-left">
              <h4 class="text-white">Works</h4>
              <p class="mb-0 text-white-50">
                HTML/CSSで作成した<br>
                コーポレートサイト（架空）です。
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
    
    <!-- work two -->
    <div class="row gx-0 mb-5 mb-lg-0 justify-content-center">
      <div class="col-lg-6">
        <a href="http://portfolio.local/works/" target="_blank">
          <img class="img-fluid work_two_img w-100" src="<?php echo esc_url(get_theme_file_uri('img/SAMURAI UNIVERSITY.png'));?>" alt="works image">  
        </a>
      </div>
      <div class="col-lg-6 order-lg-first">
        <div class="bg-black text-center h-100 work">
          <div class="d-flex h-100">
            <div class="project-text w-100 my-auto text-center text-lg-left">
              <h3 class="text-white">Works</h3>
              <p class="mb-0 text-white-50">
                jQueryを使用して作成した大学サイト（架空）です。
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>

<!--contact section  -->

<section class="contact-section" id="contact">
  <div class="container px-4 px-lg-5">
    <div class="row gx-4 gx-lg-5">
      <div class="col-md-10 col-lg-8 mx-auto text-center">
        <a href="http://portfolio.local/contact/">
          <h2 class="text-dark mb-5 ">Contact</h2>
        </a>

          
      

        <!-- <form class="form-signup" id="contactForm" data-sb-form-api-token="API_TOKEN"></form>  -->
       
        <!-- Email address input-->
        <!-- <div class="row input-group-newsletter">
          <div class="col">
            <input class="form-control" id="emailAddress" type="email" placeholder="Enter email address..." aria-label="Enter email address..." data-sb-validations="required,email" /> 
          </div>
          <div class="col-auto">
            <button class="btn btn-dark disabled" id="submitButton" type="submit">Notify Me!</button>
          </div>
          <div class="back_color"></div>
        </div> -->
      </div>
    </div>
  </div>
</section>
<!-- skill section -->
<section class="skill-section bg-black pt-5" id="skill">
  <h2 class="text-white text-center">SKILL</h2>
  <div class="container px-4 px-lg-5">
    <div class="row gx-4 gx-lg-5">
      <div class="col-md-4 mb-3 mb-md-0">
        <div class="card py-4 h-100">
          <div class="card-body text-center">
            <svg xmlns=" http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;"><path d="M4.136 3.012h15.729l-1.431 16.15-6.451 1.826-6.414-1.826-1.433-16.15zm5.266 7.302-.173-2.035 7.533.002.173-1.963-9.87-.002.522 5.998h6.835l-.243 2.566-2.179.602-2.214-.605-.141-1.58H7.691l.247 3.123L12 17.506l4.028-1.08.558-6.111H9.402v-.001z"></path></svg>
            <h4 class="m-0">HTML/CSS</h4>
            <hr class="my-4 mx-auto">
            <div class="small text-black-50">レスポンシブ対応できます</div>
          </div>
        </div>
      </div>
      
      <div class="col-md-4 mb-3 mb-md-0">
        <div class="card py-4 h-100">
          <div class="card-body text-center">
            <svg xmlns=" http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;"><path d="M3 3h18v18H3V3zm16.525 13.707c-.131-.821-.666-1.511-2.252-2.155-.552-.259-1.165-.438-1.349-.854-.068-.248-.078-.382-.034-.529.113-.484.687-.629 1.137-.495.293.09.563.315.732.676.775-.507.775-.507 1.316-.844-.203-.314-.304-.451-.439-.586-.473-.528-1.103-.798-2.126-.775l-.528.067c-.507.124-.991.395-1.283.754-.855.968-.608 2.655.427 3.354 1.023.765 2.521.933 2.712 1.653.18.878-.652 1.159-1.475 1.058-.607-.136-.945-.439-1.316-1.002l-1.372.788c.157.359.337.517.607.832 1.305 1.316 4.568 1.249 5.153-.754.021-.067.18-.528.056-1.237l.034.049zm-6.737-5.434h-1.686c0 1.453-.007 2.898-.007 4.354 0 .924.047 1.772-.104 2.033-.247.517-.886.451-1.175.359-.297-.146-.448-.349-.623-.641-.047-.078-.082-.146-.095-.146l-1.368.844c.229.473.563.879.994 1.137.641.383 1.502.507 2.404.305.588-.17 1.095-.519 1.358-1.059.384-.697.302-1.553.299-2.509.008-1.541 0-3.083 0-4.635l.003-.042z"></path></svg>
            <h4 class="m-0">Java Script</h4>
            <hr class="my-4 mx-auto">
            <div class="small text-black-50">動きのあるサイト作成できます</div>
          </div>
          
        </div>
      </div>
      <div class="col-md-4 mb-3 mb-md-0">
        <div class="card py-4 h-100">
          <div class="card-body text-center">
            <svg xmlns=" http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;"><path d="M19.891 7.788a8.966 8.966 0 0 1 1.099 4.313 8.986 8.986 0 0 1-4.47 7.771l2.746-7.939c.513-1.282.684-2.309.684-3.219a7.165 7.165 0 0 0-.059-.926m-6.651.087a14.14 14.14 0 0 0 1.026-.088c.485-.063.428-.775-.056-.749 0 0-1.463.112-2.4.112-.887 0-2.375-.125-2.375-.125-.487-.024-.55.713-.061.738 0 0 .449.052.938.075l1.399 3.838-1.975 5.899-3.274-9.724a17.006 17.006 0 0 0 1.028-.083c.487-.063.43-.775-.055-.747 0 0-1.455.115-2.395.115-.167 0-.365-.007-.575-.013C6.093 4.726 8.862 3.113 12 3.113c2.341 0 4.471.894 6.071 2.36-.038-.002-.076-.008-.117-.008-.883 0-1.51.77-1.51 1.596 0 .741.427 1.369.883 2.108.343.601.742 1.37.742 2.481 0 .763-.295 1.662-.685 2.899l-.896 2.987-3.25-9.675.002.014zM12 21.087a8.983 8.983 0 0 1-2.54-.364l2.697-7.838 2.763 7.572c.021.044.042.085.065.124a9.016 9.016 0 0 1-2.985.508m-8.99-8.988a8.94 8.94 0 0 1 .778-3.658l4.287 11.749a8.993 8.993 0 0 1-5.065-8.091m8.99-10c-5.513 0-10 4.487-10 10s4.487 10 10 10 10-4.487 10-10-4.487-10-10-10"></path></svg>
            <h4 class="m-0">Word Press</h4>
            <hr class="my-4 mx-auto">
            <div class="small text-black-50">既存のhtmlファイルから変換可能です</div>
          </div>
        </div>
      </div>
    </div>
    <div class="social d-flex justify-content-center">
      <a class="mx-2" href="#"><svg xmlns=" http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;"><path d="M19.633 7.997c.013.175.013.349.013.523 0 5.325-4.053 11.461-11.46 11.461-2.282 0-4.402-.661-6.186-1.809.324.037.636.05.973.05a8.07 8.07 0 0 0 5.001-1.721 4.036 4.036 0 0 1-3.767-2.793c.249.037.499.062.761.062.361 0 .724-.05 1.061-.137a4.027 4.027 0 0 1-3.23-3.953v-.05c.537.299 1.16.486 1.82.511a4.022 4.022 0 0 1-1.796-3.354c0-.748.199-1.434.548-2.032a11.457 11.457 0 0 0 8.306 4.215c-.062-.3-.1-.611-.1-.923a4.026 4.026 0 0 1 4.028-4.028c1.16 0 2.207.486 2.943 1.272a7.957 7.957 0 0 0 2.556-.973 4.02 4.02 0 0 1-1.771 2.22 8.073 8.073 0 0 0 2.319-.624 8.645 8.645 0 0 1-2.019 2.083z"></path></svg></a>
      <a class="mx-2" href="#"><svg xmlns="<?php get_template_directory_uri(  );?> http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;"><path d="M13.397 20.997v-8.196h2.765l.411-3.209h-3.176V7.548c0-.926.258-1.56 1.587-1.56h1.684V3.127A22.336 22.336 0 0 0 14.201 3c-2.444 0-4.122 1.492-4.122 4.231v2.355H7.332v3.209h2.753v8.202h3.312z"></path></svg></a>
      <a class="mx-2" href="#"><svg xmlns=" http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;"><path fill-rule="evenodd" clip-rule="evenodd" d="M12.026 2c-5.509 0-9.974 4.465-9.974 9.974 0 4.406 2.857 8.145 6.821 9.465.499.09.679-.217.679-.481 0-.237-.008-.865-.011-1.696-2.775.602-3.361-1.338-3.361-1.338-.452-1.152-1.107-1.459-1.107-1.459-.905-.619.069-.605.069-.605 1.002.07 1.527 1.028 1.527 1.028.89 1.524 2.336 1.084 2.902.829.091-.645.351-1.085.635-1.334-2.214-.251-4.542-1.107-4.542-4.93 0-1.087.389-1.979 1.024-2.675-.101-.253-.446-1.268.099-2.64 0 0 .837-.269 2.742 1.021a9.582 9.582 0 0 1 2.496-.336 9.554 9.554 0 0 1 2.496.336c1.906-1.291 2.742-1.021 2.742-1.021.545 1.372.203 2.387.099 2.64.64.696 1.024 1.587 1.024 2.675 0 3.833-2.33 4.675-4.552 4.922.355.308.675.916.675 1.846 0 1.334-.012 2.41-.012 2.737 0 .267.178.577.687.479C19.146 20.115 22 16.379 22 11.974 22 6.465 17.535 2 12.026 2z"></path></svg></a>
    </div>
  </div>
</section>
</body>
<?php get_footer(  );?>
